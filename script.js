...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 카카오 로그인 콜백 확인 (팝업에서 리다이렉트 후 토큰 처리)
    checkKakaoLoginCallback();
    ...
    // 팝업으로부터 네이버 로그인 완료 메시지 수신 리스너 등록
    window.addEventListener('message', (event) => {
        if (event.origin !== window.location.origin) return;
        
        if (event.data && event.data.type === 'NAVER_LOGIN_SUCCESS') {
            const { userName, userPhone } = event.data;
        ...
        // 카카오 OAuth 팝업 완료 메시지 수신
        if (event.data && event.data.type === 'KAKAO_LOGIN_SUCCESS') {
            const { userName, userPhone, isRegister } = event.data;
            currentUserRole = 'user';
            currentUserName = userName;
            currentUserPhone = userPhone;
...
async function requestTossPayment() {
    ...
    try {
        ...
        // 폰 컨테이너 내부로 강제 이동시켰던 토스페이먼츠 엘리먼트를 수동으로 모두 제거
        // (DOM 위치 변경으로 인해 Toss SDK가 자체 UI 제거에 실패하고 종료된 세션 화면이 남는 현상 방지)
        const phoneContainer = document.querySelector('.phone-container');
        if (phoneContainer) {
            Array.from(phoneContainer.children).forEach(node => {
                const id = node.id || '';
                const className = typeof node.className === 'string' ? node.className : '';
                const styleAttr = node.getAttribute('style') || '';
                
                const isIframe = node.tagName === 'IFRAME';
                let hasTossIframe = false;
                if (node.querySelector) {
                    const iframe = node.querySelector('iframe');
                    if (iframe && (iframe.src.includes('toss') || iframe.id.includes('toss') || iframe.name.includes('toss'))) {
                        hasTossIframe = true;
                    }
                }
                ...
                if (
                    ...
                    (isIframe && ((node.src && node.src.includes('toss')) || id.includes('toss') || node.name.includes('toss'))) ||
                    hasTossIframe
                ) {
                    node.remove();
                }
...
function showCancelConfirmModal() {
    ...
    overlay.id = 'cancel-confirm-modal';
    ...
    modalBox.style.transition = 'transform 0.2s cubic-bezier(0.175, 0.885, 0.32, 1.275)';

    const icon = document.createElement('span');
    icon.className = 'material-icons-round';
    icon.textContent = 'error_outline';
    icon.style.fontSize = '36px';
    icon.style.color = '#D38E32';
    icon.style.marginBottom = '12px';
    icon.style.display = 'block';

    const title = document.createElement('h3');
    title.textContent = '결제를 취소하시겠습니까?';
    ...
    btnYes.style.border = '1px solid #E0E0E0';
    btnYes.style.backgroundColor = '#FFF';
    btnYes.style.color = '#333';
    btnYes.style.fontSize = '15px';
    ...
    btnYes.onclick = () => {
        ...
        setTimeout(() => {
            overlay.remove();
            showCustomToast('장바구니로 돌아왔습니다.', 'info');
        }, 200);
    };
    ...
    btnNo.style.backgroundColor = '#D38E32';
    btnNo.style.color = '#FFF';
    btnNo.style.fontSize = '15px';
...
function initTossIframeObserver() {
    ...
    // 토스 SDK가 DOM 이동으로 인해 엘리먼트를 찾지 못하고 DOMException 에러를 발생시키며 먹통이 되는 현상(프로미스 행 걸림)을 방지
    if (!document.body._tossPatched) {
        document.body._tossPatched = true;
        const originalRemoveChild = document.body.removeChild;
        document.body.removeChild = function(child) {
            if (child && child.parentNode && child.parentNode !== document.body) {
                return child.parentNode.removeChild(child);
            }
            return originalRemoveChild.call(this, child);
        };
        const originalContains = document.body.contains;
        document.body.contains = function(child) {
            if (child && child.parentNode && child.parentNode.classList && child.parentNode.classList.contains('phone-container')) {
                return true;
            }
            return originalContains.call(this, child);
        };
    }
...
// 카카오 로그인 실시간 연동 콜백 확인 함수 (팝업 창에서 토큰 수신 후 부모에 전달)
function checkKakaoLoginCallback() {
    ...
    const isRegister = sessionStorage.getItem('kakao_oauth_is_register') === 'true';

    if (typeof Kakao !== 'undefined') {
        // SDK로 토큰 설정 후 프로필 요청
        if (!Kakao.isInitialized()) Kakao.init('23e243ab8d838a096da972987027a26f');
        Kakao.Auth.setAccessToken(accessToken);

        Kakao.API.request({
            url: '/v2/user/me',
            success: function(res) {
                const nickname = (res.kakao_account && res.kakao_account.profile && res.kakao_account.profile.nickname)
                    ? res.kakao_account.profile.nickname : '카카오 회원';
                const mobile = (res.kakao_account && res.kakao_account.phone_number)
                    ? res.kakao_account.phone_number : 'SNS_Kakao';

                // URL 해시 정리
                history.replaceState(null, null, window.location.pathname + window.location.search);

                // 팝업에서 호출된 경우 부모 창에 메시지 전달 후 자동 닫힘
                if (window.opener) {
            ...
            fail: function(err) {
                console.error('카카오 프로필 요청 실패:', err);
                history.replaceState(null, null, window.location.pathname + window.location.search);
...
/**
 * 카카오 OAuth 2.0 팝업 창을 열고 로그인/가입 흐름을 시작합니다.
 * @param {boolean} isRegister - true: 회원가입, false: 로그인
 */
function _openKakaoOAuthPopup(isRegister) {
        ...
        if (typeof naver !== 'undefined' && naver.LoginWithNaverId) {
            ...
            naverLogin.getLoginStatus(function (status) {
                if (status) {
                    ...
                    // 만약 팝업창으로 가동 중이라면, 부모 창으로 로그인 정보를 전달하고 스스로 닫힙니다.
                    if (window.opener) {
                        window.opener.postMessage({
...
function executeNaverMockFlow(isRegister) {
    const providerName = '네이버';
    const userName = '네이버 붕어짱';
    const themeColor = '#03C75A';
    ...
    `;
    loadingOverlay.innerHTML = `
        <div style="width: 80px; height: 80px; border-radius: 50%; background-color: ${themeColor}; display: flex; align-items: center; justify-content: center; margin-bottom: 20px; box-shadow: 0 8px 24px rgba(3,199,90,0.4);">
            <svg viewBox="0 0 24 24" style="width: 30px; height: 30px; fill: #FFFFFF;">
                <path d="M16.2 3H21v18h-4.8l-8.4-12.3V21H3V3h4.8l8.4 12.3V3z"/>
            </svg>
        </div>
        <h3 style="font-size: 20px; font-weight: 800; color: #3E2723; margin: 0 0 8px 0; font-family: inherit;">${providerName} 우회 간편 연동 중</h3>
        <p style="font-size: 14px; font-weight: 700; color: #8D6E63; margin: 0 0 24px 0; font-family: inherit;">콘솔 설정 오류를 우회하여 가상 계정 정보로 로그인 중입니다.</p>
    ...
    setTimeout(() => {
        ...
        setTimeout(() => {
            ...
            alert(`🎉 ${userName}님으로 안전하게 간편 ${isRegister ? '가입 및 로그인' : '로그인'}되었습니다!`);
            updateSettingsUI();
            navigateTo('scr-dough');
        }, 300);
    }, 1200);
...
function handleSnsLogin(provider) {
    ...
    if (provider === 'naver') {
        // 폰 목업 안의 인앱 네이버 로그인 화면으로 이동
        sessionStorage.setItem('naver_login_return_screen', 'scr-login');
        sessionStorage.setItem('naver_login_is_register', 'false');
        navigateTo('scr-naver-login');
        setTimeout(() => {
...
function handleSnsRegister(provider) {
    ...
    if (provider === 'naver') {
        // 폰 목업 안의 인앱 네이버 로그인 화면으로 이동 (회원가입 플로우)
        sessionStorage.setItem('naver_login_return_screen', 'scr-register');
        sessionStorage.setItem('naver_login_is_register', 'true');
        navigateTo('scr-naver-login');
        setTimeout(() => {
            const idInput = document.getElementById('naver-inapp-id');
            if (idInput) idInput.focus();
...
/**
 * 네이버 인앱 로그인 화면의 뒤로가기 버튼 클릭 시 호출
 */
function naverLoginBack() {
    // 입력 필드 초기화
    const idEl = document.getElementById('naver-inapp-id');
    const pwEl = document.getElementById('naver-inapp-pw');
    if (idEl) idEl.value = '';
    if (pwEl) pwEl.value = '';

    const returnScreen = sessionStorage.getItem('naver_login_return_screen') || 'scr-login';
    navigateTo(returnScreen);
}

/**
 * 네이버 인앱 로그인 화면의 비밀번호 표시/숨김 토글
 */
function toggleNaverPw(iconEl) {
    const pwInput = document.getElementById('naver-inapp-pw');
    if (!pwInput) return;
...
function submitNaverInAppLogin() {
    const idInput = document.getElementById('naver-inapp-id');
    const pwInput = document.getElementById('naver-inapp-pw');
    const submitBtn = document.getElementById('naver-login-submit-btn');

    const id = idInput ? idInput.value.trim() : '';
    const pw = pwInput ? pwInput.value.trim() : '';

    if (!id) {
        showCustomToast('아이디 또는 전화번호를 입력해주세요.', 'person');
        if (idInput) idInput.focus();
        return;
    }
    if (!pw) {
...
/**
 * 네이버 인앱 로그인 입력 변경 감지 → 버튼 활성화
 * HTML에서 oninput으로 연결
 */
function onNaverInputChange() {
    const id = (document.getElementById('naver-inapp-id') || {}).value || '';
    const pw = (document.getElementById('naver-inapp-pw') || {}).value || '';
    const btn = document.getElementById('naver-login-submit-btn');
    if (btn) {
        if (id.trim() && pw.trim()) {
